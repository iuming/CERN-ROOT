...
#include "TAxis.h"
#include "RooPlot.h"
#include "RooFitResult.h"           // RooFitResult 是一个容器类，用于保存适合数据集的 PDF 的输入和输出
#include "RooGenericPdf.h"
#include "RooConstVar.h"            // 表示一个常量实值对象
...
void rf103_interprfuncs()
{
   ...
   RooGenericPdf genpdf("genpdf", "genpdf", "(1+0.1*abs(x)+sin(sqrt(abs(x*alpha+0.1))))", RooArgSet(x, alpha));   // 自定义概论密度函数
   ...
   RooGaussian g1("g1", "g1", x, RooConst(10), RooConst(3));   // 定义固定的高斯概率密度函数
   ...
   RooFitResult *r = g2.fitTo(*data2, Save());     // 采用g2函数拟合date2数据，存储在r中
   r->Print();
